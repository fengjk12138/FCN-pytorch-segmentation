...
class FCN32(nn.Module):
    def __init__(self):
        ...
        self.classifier.weight = nn.Parameter(torch.zeros(self.classifier.weight.data.size()),  requires_grad=True)
        self.classifier.bias = nn.Parameter(torch.zeros(self.classifier.bias.size()),  requires_grad=True)
    ...
    def forward(self, x):
        ...
        h = self.upsample(after_class)
        h = h[:, :, 19:19 + x.size()[2], 19:19 + x.size()[3]].contiguous()
        return h

    def init_vgg_para(self):
        tmp_para = torchvision.models.vgg16(pretrained=True)
        for l1, l2 in zip(tmp_para.features, [*zip(self.feature), *zip(self.feature2), *zip(self.feature3)]):
            l2 = l2[0]
            if isinstance(l1, nn.Conv2d) and isinstance(l2, nn.Conv2d):
                if l1.weight.size() != l2.weight.size():
                    print("error")
                    exit(0)
                # l2.weight = nn.Parameter(l1.weight.data, requires_grad=True)
                # l2.bias = nn.Parameter(l1.bias.data, requires_grad=True)

                l2.weight.data = l1.weight.data
                l2.bias.data = l1.bias.data
        self.fc6[0].weight.data = tmp_para.classifier[0].weight.data.view(self.fc6[0].weight.data.size())
        self.fc7[0].weight.data = tmp_para.classifier[3].weight.data.view(self.fc7[0].weight.data.size())
        self.fc6[0].bias.data = tmp_para.classifier[0].bias.data.view(self.fc6[0].bias.data.size())
        self.fc7[0].bias.data = tmp_para.classifier[3].bias.data.view(self.fc7[0].bias.data.size())

        initial_weight = get_upsampling_weight(
            self.upsample.in_channels, self.upsample.out_channels, self.upsample.kernel_size[0])
        self.upsample.weight = nn.Parameter(initial_weight, requires_grad=True)
